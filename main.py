        ...
        with c61:
            ...
            for i, v in enumerate(x):
                ...
                plt.text(v, y[i], f'{actual_y[i]:.1f}'+'%',
                        ...
                        verticalalignment='bottom')
            st.pyplot(fig)
            e_t = e_news_p[0]-e_news_p[1]
            s_t = s_news_p[0]-s_news_p[1]
            g_t = g_news_p[0]-g_news_p[1]
            e_txt = f'{abs(e_t):.2f}%p 낮고' if e_t<0 else ('동일하고' if e_t==0 else f'{abs(e_t):.2f}%p 높고')
            s_txt = f'{abs(s_t):.2f}%p 낮으며' if s_t<0 else ('동일하며' if s_t==0 else f'{abs(s_t):.2f}%p 높으며')
            g_txt = f'{abs(g_t):.2f}%p 낮습니다' if g_t<0 else ('동일합니다' if g_t==0 else f'{abs(g_t):.2f}%p 높습니다')

            st.write(f"{com}은 산업평균 대비<br><strong>E(Environment)</strong> 뉴스 비율이 {e_txt}, <br><strong>S(Social)</strong> 뉴스 비율이 {s_txt}, <br><strong>G(Governance)</strong> 뉴스 비율은 {g_txt}.<br></h5>", unsafe_allow_html = True)
